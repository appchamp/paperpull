"""Chase credit-card statement downloader (local, supervised).

Usage:
    python chase_docs.py --login       verify connection to your browser
    python chase_docs.py --discover    list available documents
    python chase_docs.py --pilot       download the 5 newest, then stop
    python chase_docs.py --all         download everything in scope
    python chase_docs.py --resume      continue an interrupted run
    python chase_docs.py --verify      re-validate every saved PDF
    python chase_docs.py --diagnose    dump page structure (no downloads)
    python chase_docs.py --dry-run     plan filenames, save nothing
...
Chase already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("chase_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # Chase's stable per-document UUID
        # Chase labels several statements per date identically; `occurrence` is
    ...
    @property
    def key(self) -> str:
        """Stable identity. Chase's API gives each document a durable
...
class App:
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Chase browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Chase shows a signed-out page.")
    ...
    def _record_chase_doc(self, d: dict) -> int:
        ...
        if account:
            ...
            # Two documents Chase describes identically on the same date. We
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        if not site.ensure_statements(page):
            ...
            if not site.ensure_statements(page):
                print("Could not open your Chase statements. Sign in and open")
                print("Statements & Documents in the browser, then try again.")
                return 0
        self.check_session(page)

        # Chase's document list is read from the page until a probe confirms
        ...
        if raw:
            ...
            log.info("Chase: API returned nothing - falling back to scraping")
            print("  Chase's document API returned nothing; reading the page instead.")
            raw = site.chase_collect(page)
            log.info("Chase: collected %d document rows", len(raw))
        n_new = 0
        for d in raw:
            n_new += self._record_chase_doc(d)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        # Reach the statements page (reuse the signed-in tab); chase_download
        ...
        saved = site.chase_download(page, page.context, doc.account, doc.date,
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Chase documents ({scope}).")
        ...
        try:
            ...
            try:
                ...
                info["account_options"] = [re.sub(r"\$[\d,.]+", "$...", a)
                                           for a in accounts]
                # Chase's real pickers: card accordions and the styled
                # "View:" year input (neither is a <select>).
                info["cards"] = [label for _el, label in site.card_accordions(page)]
                info["year_options"] = site.year_options(page) or years
            except Exception as e:
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Chase document downloader (read-only)")
