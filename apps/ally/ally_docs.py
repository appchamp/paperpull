"""Ally statement & tax-document downloader (local, supervised).

Usage:
    python ally_docs.py --login       verify connection to your browser
    python ally_docs.py --discover    list available documents
    python ally_docs.py --pilot       download the 5 newest, then stop
    python ally_docs.py --all         download everything in scope
    python ally_docs.py --resume      continue an interrupted run
    python ally_docs.py --verify      re-validate every saved PDF
    python ally_docs.py --diagnose    dump page structure (no downloads)
    python ally_docs.py --dry-run     plan filenames, save nothing
...
Ally already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("ally_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # Ally's stable per-document UUID
        # Ally labels several statements per date identically; `occurrence` is
        ...
        # "statement" or "tax" - they live in different tabs and are found
        # different ways (a statement by date+position, a form by its name).
        self.kind = kind
    ...
    @property
    def key(self) -> str:
        """Stable identity. Ally's API gives each document a durable
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Ally appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Ally browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Ally shows a signed-out page.")
    ...
    def _record_ally_doc(self, d: dict) -> int:
        """Record one collected statement. Returns 1 if new.

        Accepts both shapes: the API's {documentId, date, title, account,
        label} and the DOM fallback's {account, date, title}.
        """
        ...
        if not doc_types.wanted(category, self.config):
            self.stats["skipped_out_of_scope"] += 1
            return 0
        self.stats.setdefault("registrations", {})
        date = (d.get("date") or "").strip()
        floor = self.args.start_date or self.config.get("default_start_date")
        if floor and (not date or date < floor):
            self.stats["skipped_out_of_scope"] += 1
            return 0
        # `account` is the full ownership name (Ally's trustName) - it is how a
        # row is re-found at download time. `label` is the short form that goes
        # in the filename, so no personal name is written to disk.
        account = re.sub(r"\s+", " ", (d.get("account") or "")).strip()
        label = (d.get("label") or "").strip()
        if not label:
            label = site.account_label(account, bool(d.get("ira")),
                                       self.config.get("account_labels"))
        doc_id = d.get("documentId", "")
        if d.get("corrected"):
            # keep it next to the original in a sorted listing, but distinct
            summary = f"{summary} (Corrected)"
        if label:
            full_summary = f"{summary} - {label}"
        elif d.get("ambiguous"):
            # Ally posts several statements on this date and describes them
            # identically - no registration, same title, same row label. We
        ...
        if account:
            reg = self.stats["registrations"].setdefault(
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        # a durable documentId per statement - a far better identity than a
        # scraped row, and it survives a redesign of the table. Scraping is the
        # fallback for when that API answers nothing.
        labels = self.config.get("account_labels") or {}
        raw = site.ally_collect_via_api(page, labels)
        if raw:
            log.info("Ally: %d statements read from the API", len(raw))
        else:
            log.info("Ally: API returned nothing - falling back to row scraping")
            print("  Ally's statements API returned nothing; reading the page instead.")
            raw = site.ally_collect(page)
            log.info("Ally: collected %d document rows", len(raw))
        n_new = 0
        for d in raw:
            n_new += self._record_ally_doc(d)
        # Tax forms live in their own tab on the same page, behind the same
        # API with a different docType. Discovery opens that tab and captures
        # whatever it calls, rather than assuming the parameter's value.
        if doc_types.wanted(doc_types.TAX, self.config):
            try:
                tax = site.ally_collect_tax_via_api(page)
            except Exception as e:
                log.info("tax discovery failed: %s", e)
                tax = []
            if tax:
                log.info("Ally: %d tax form(s) read from the API", len(tax))
                for d in tax:
                    n_new += self._record_ally_doc(d)
            else:
                print("  No tax forms found (Ally posts them seasonally).")
        ...
        if not quiet:
            ...
            # Ally's metadata names only some statements, so show what it did
            # and did not tell us. The final filename comes from each PDF's own
            ...
            if regs or un:
                ...
                for name, info in sorted(regs.items(), key=lambda kv: -kv[1]["count"]):
                    ira = "  [Ally flags this one as an IRA]" if info["ira"] else ""
                    print(f"    {info['count']:4}  registered to \"{name}\"{ira}")
                if un:
                    print(f"    {un:4}  no registration given - which account these "
                          f"cover is only knowable from the PDF")
                mapped = (self.config.get("account_labels") or {})
                unmapped = [n for n in regs if n not in mapped]
                if unmapped:
                    print("\n  If a PDF's layout ever fails to parse, the fallback name")
                    print("  uses the registration above. To shorten it, map it in")
                    print("  config.json (nothing is guessed for you):")
                    print('      "account_labels": {')
                    print(",\n".join(f'          "{n}": "<your label>"' for n in unmapped))
                    print("      }")
    ...
    def _name_from_contents(self, doc: Document, out_path: Path) -> Path:
        """Rename a downloaded statement after what the PDF itself says.

        Ally describes several statements per date identically, so its
        metadata cannot say which accounts a given PDF covers - the PDF can.
        The accounts (and, where two copies of one statement differ only by
        ...
        """
        if doc.kind == "tax":
            # A tax form has no account-summary table; its own name ("1099-INT")
            # already says what it is, so discovery's name stands.
            return out_path
        try:
            facts = site.read_statement_facts(out_path)
        except Exception as e:                       # never lose a good PDF to this
        ...
        # Keep the document type ("Account Statement") in front - the accounts
        # say WHICH statement, not what kind of document it is.
        _cat, kind, _conf = doc_types.classify_document(doc.title, self.rules)
        summary = f"{kind} - {facts.account_summary(70)}"
        if self.config.get("addressee_in_filename", True) and facts.addressee:
            summary = f"{summary} ({site.normalize_name(facts.addressee)[:40]})"
        elif doc.ambiguous and doc.document_id:
            # Without the addressee, two copies of one statement would collide.
            summary = f"{summary} - {doc.document_id[-4:]}"

        doc.account = facts.addressee or doc.account
        doc.summary = summary
        filename = build_pdf_filename(doc.date, summary, "")
        if filename == out_path.name:
            return out_path
        target = unique_path(out_path.parent, filename,
                             self.config["max_path_length"])
        try:
            out_path.replace(target)
        except OSError as e:
            log.info("could not rename %s: %s", out_path.name, e)
            return out_path
        log.info("named from contents: %s", target.name)
        return target
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        saved = site.ally_download(page, page.context, doc.account, doc.date,
        ...
        out_path = self._name_from_contents(doc, out_path)
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Ally documents ({scope}).")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Ally document downloader (read-only)")
