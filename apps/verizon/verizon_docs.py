...
Usage:
    ...
    python verizon_docs.py --discover    list available documents
    python verizon_docs.py --pilot       download the 5 newest, then stop
    python verizon_docs.py --all         download everything in scope
    python verizon_docs.py --resume      continue an interrupted run
    python verizon_docs.py --verify      re-validate every saved PDF
    python verizon_docs.py --diagnose    dump page structure (no downloads)
    python verizon_docs.py --dry-run     plan filenames, save nothing
...
Verizon already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("verizon_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # Verizon's stable per-document UUID
    ...
    @property
    def key(self) -> str:
        """Stable identity. Verizon's API gives each document a durable
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in Verizon (Edge) tab. A fresh tab
            # would be unauthenticated and would also trip Verizon's bot block.
        ...
        # Downloads (from the real Edge/Chrome we attach to) bypass Playwright's
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Verizon appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Verizon browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Verizon shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has Verizon documents from 2023 and earlier).
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a Verizon page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Verizon lists every bill on one paginated billing-history page. Open
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        download link and clicking it (Verizon fires a real download event)."""
        self.check_session(page)
        ...
        # Open the "Download Your Bill" page, pick this bill's date + Download
        # PDF + Get My Bill; the PDF lands in the controlled download dir and is
        # moved to out_path.
        if not site.goto_documents(page):
            self.check_session(page)
            site.goto_documents(page)
        saved = site.download_bill(page, self._vz_dl_dir, doc.date, out_path)
        if not saved:
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Verizon documents ({scope}).")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Verizon document downloader (read-only)")
