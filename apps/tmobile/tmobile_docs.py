...
Usage:
    ...
    python tmobile_docs.py --discover    list available documents
    python tmobile_docs.py --pilot       download the 5 newest, then stop
    python tmobile_docs.py --all         download everything in scope
    python tmobile_docs.py --resume      continue an interrupted run
    python tmobile_docs.py --verify      re-validate every saved PDF
    python tmobile_docs.py --diagnose    dump page structure (no downloads)
    python tmobile_docs.py --dry-run     plan filenames, save nothing
...
T-Mobile already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("tmobile_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # T-Mobile's stable per-document UUID
    ...
    @property
    def key(self) -> str:
        """Stable identity. T-Mobile's API gives each document a durable
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! T-Mobile appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9231")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in T-Mobile browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but T-Mobile shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has T-Mobile documents from 2023 and earlier).
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a T-Mobile page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # T-Mobile lists every bill on one paginated billing-history page. Open
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available T-Mobile documents ({scope}).")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "T-Mobile Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised T-Mobile document downloader (read-only)")
