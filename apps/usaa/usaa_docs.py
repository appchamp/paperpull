"""USAA statement & tax-document downloader (local, supervised).

Usage:
    python usaa_docs.py --login       verify connection to your browser
    python usaa_docs.py --discover    list available documents
    python usaa_docs.py --pilot       download the 5 newest, then stop
    python usaa_docs.py --all         download everything in scope
    python usaa_docs.py --resume      continue an interrupted run
    python usaa_docs.py --verify      re-validate every saved PDF
    python usaa_docs.py --diagnose    dump page structure (no downloads)
    python usaa_docs.py --dry-run     plan filenames, save nothing
...
USAA already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("usaa_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # USAA's stable per-document UUID
    ...
    @property
    def key(self) -> str:
        """Stable identity. USAA's API gives each document a durable
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! USAA appears to have signed you out.")
    ...
    def cmd_login(self):
        ...
        if challenge:
            ...
            print("Connected, but USAA shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has USAA documents from 2023 and earlier).
    ...
    def _record_api_doc(self, d: dict) -> int:
        """Record one document dict from USAA's documents API. Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        if not site.goto_documents(page):
            ...
            if not site.goto_documents(page):
                print("Could not open your USAA documents. Sign in and open your")
        ...
        log.info("USAA documents API returned %d unique documents", len(api_docs))
        n_new = 0
        for d in api_docs:
            n_new += self._record_api_doc(d)
        self.discovery.save()
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available USAA documents ({scope}).")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised USAA document downloader (read-only)")
