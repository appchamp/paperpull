"""Paylocity statement & tax-document downloader (local, supervised).

Usage:
    python paylocity_docs.py --login       verify connection to your browser
    python paylocity_docs.py --discover    list available documents
    python paylocity_docs.py --pilot       download the 5 newest, then stop
    python paylocity_docs.py --all         download everything in scope
    python paylocity_docs.py --resume      continue an interrupted run
    python paylocity_docs.py --verify      re-validate every saved PDF
    python paylocity_docs.py --diagnose    dump page structure (no downloads)
    python paylocity_docs.py --dry-run     plan filenames, save nothing
...
Paylocity already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("paylocity_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # Paylocity's stable per-document UUID
    ...
    @property
    def key(self) -> str:
        """Stable identity. Paylocity's API gives each document a durable
...
class App:
    def __init__(self, args):
        ...
        # Paylocity has no single address: every employer runs its own tenant, so
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in Paylocity tab. A fresh tab would
            # be unauthenticated (Paylocity keeps the session in that tab).
        ...
        # Paylocity fires ordinary download events that Playwright's
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Paylocity appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        print("Sign in at access.paylocity.com with your Company ID, username and")
        print("password (or your company SSO). Keep the window OPEN, then run the pilot.")

    def cmd_login(self):
        print("Checking the connection to your signed-in Paylocity browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Paylocity shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has Paylocity documents from 2023 and earlier).
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a Paylocity page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Paylocity lists every bill on one paginated billing-history page. Open
        # it (cookie session survives the nav) and read the statement date from
        # each accordion panel across all pages.
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        download link and clicking it (Paylocity fires a real download event)."""
        self.check_session(page)
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Paylocity documents ({scope}).")
    ...
    def cmd_diagnose(self):
        ...
        try:
            ...
            # This provider is API-driven, so diagnose shows what the JSON
            # endpoints return, not a DOM scrape. Amounts are never recorded,
            # so the sample below carries only title, date and classification.
            info["pay_assignments"] = len(site.pay_assignments(page))
            docs = site.collect_documents(page)
            ...
            for d in docs[:8]:
                ...
                info["samples"].append({
                    "title": d.title[:90], "date": d.date_text,
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Paylocity Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Paylocity document downloader (read-only)")
