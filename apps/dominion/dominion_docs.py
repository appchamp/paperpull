...
Usage:
    ...
    python dominion_docs.py --discover    list available documents
    python dominion_docs.py --pilot       download the 5 newest, then stop
    python dominion_docs.py --all         download everything in scope
    python dominion_docs.py --resume      continue an interrupted run
    python dominion_docs.py --verify      re-validate every saved PDF
    python dominion_docs.py --diagnose    dump page structure (no downloads)
    python dominion_docs.py --dry-run     plan filenames, save nothing
...
Dominion already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("dominion_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # Dominion's stable per-document UUID
    ...
    @property
    def key(self) -> str:
        """Stable identity. Dominion's API gives each document a durable
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Dominion appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Dominion browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Dominion shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has Dominion documents from 2023 and earlier).
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a Dominion page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Dominion lists every bill on one paginated billing-history page. Open
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        download link and clicking it (Dominion fires a real download event)."""
        self.check_session(page)
        ...
        # Open the billing-history page, then expand this bill's row and click
        # its 'Download Your Detailed Bill PDF' button (download_statement walks
        # the pagination to find the matching statement date).
        if not site.goto_documents(page):
            self.check_session(page)
            site.goto_documents(page)
        saved = site.download_statement(page, doc.date, out_path)
        if not saved:
        ...
        # Dominion returns a placeholder PDF for bills older than ~18 months.
        if site.is_unavailable_bill(out_path):
            try:
                out_path.unlink()
            except OSError:
                pass
            self._record(doc, State.NO_RECEIPT_AVAILABLE,
                         notes="Bill PDF not available online (Dominion keeps ~18 months)")
            self._write_row(doc, "Not available (older than ~18 months)",
                            "No Receipt Available")
            self.stats["no_receipt_available"] = \
                self.stats.get("no_receipt_available", 0) + 1
            print("  Bill PDF not available online (older than ~18 months) - "
                  "marked unavailable, will not retry.")
            return
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Dominion documents ({scope}).")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Dominion Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Dominion document downloader (read-only)")
