...
log = logging.getLogger("schwab_docs")
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Charles Schwab appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Charles Schwab browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Charles Schwab shows a signed-out page.")
    ...
    def _record_schwab_doc(self, d: dict, charitable_ids: set) -> int:
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        if not site.ensure_statements(page):
            ...
            if not site.ensure_statements(page):
                print("Could not open your Schwab statements page. Sign in in the")
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Charles Schwab documents ({scope}).")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Charles Schwab Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Charles Schwab document downloader (read-only)")
