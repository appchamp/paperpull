...
Usage:
    ...
    python redcard_docs.py --discover    list available documents
    python redcard_docs.py --pilot       download the 5 newest, then stop
    python redcard_docs.py --all         download everything in scope
    python redcard_docs.py --resume      continue an interrupted run
    python redcard_docs.py --verify      re-validate every saved PDF
    python redcard_docs.py --diagnose    dump page structure (no downloads)
    python redcard_docs.py --dry-run     plan filenames, save nothing
...
downloads PDFs that Target Circle Card already generated. It never pays a bill,
...
log = logging.getLogger("redcard_docs")
...
class Document:
    """One Target Circle Card document."""
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # Target Circle Card's stable per-document UUID
    ...
    @property
    def key(self) -> str:
        """Stable identity. Target Circle Card's API gives each document a durable
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! The Target Circle Card portal appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9232")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Target Circle Card browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Target Circle Card shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a Target Circle Card page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # RedCard holds the session in an in-memory token, so we reach the
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Target Circle Card documents ({scope}).")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Target Circle Card Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Target Circle Card document downloader (read-only)")
