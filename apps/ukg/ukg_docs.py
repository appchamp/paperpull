"""UKG statement & tax-document downloader (local, supervised).

Usage:
    python ukg_docs.py --login       verify connection to your browser
    python ukg_docs.py --discover    list available documents
    python ukg_docs.py --pilot       download the 5 newest, then stop
    python ukg_docs.py --all         download everything in scope
    python ukg_docs.py --resume      continue an interrupted run
    python ukg_docs.py --verify      re-validate every saved PDF
    python ukg_docs.py --diagnose    dump page structure (no downloads)
    python ukg_docs.py --dry-run     plan filenames, save nothing
...
UKG already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("ukg_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # UKG's stable per-document UUID
    ...
    @property
    def key(self) -> str:
        """Stable identity. UKG's API gives each document a durable
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! UKG appears to have signed you out.")
    ...
    def cmd_login(self):
        ...
        if challenge:
            ...
            print("Connected, but UKG shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has UKG documents from 2023 and earlier).
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a UKG page.
        Returns 1 if new."""
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        download link and clicking it (UKG fires a real download event)."""
        self.check_session(page)
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available UKG documents ({scope}).")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised UKG document downloader (read-only)")
