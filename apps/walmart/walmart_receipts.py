"""Walmart purchase-history & receipt downloader (local, supervised).

Usage:
    python walmart_receipts.py --login
    python walmart_receipts.py --discover
    python walmart_receipts.py --pilot            (5 newest Online + 3 newest In-store)
    python walmart_receipts.py --pilot-online
    python walmart_receipts.py --pilot-instore
    python walmart_receipts.py --online
    python walmart_receipts.py --instore
    python walmart_receipts.py --all
    python walmart_receipts.py --resume
    python walmart_receipts.py --verify
    python walmart_receipts.py --review-names
    python walmart_receipts.py --diagnose [--order-number N]
    python walmart_receipts.py --dry-run
...
class App:
    ...
    def browser(self):
        ...
        Walmart uses aggressive bot detection (PerimeterX) that blocks
        ...
        if cdp_url:
            ...
            # window Walmart opens to print also has window.print() intercepted
    ...
    def cmd_login(self):
        if self.config.get("cdp_url"):
            ...
            print("Checking the connection to your signed-in Walmart browser...\n")
            page = self.page()
            site.goto_orders(page)
            challenge = site.detect_security_challenge(page)
            if challenge:
                ...
                print("Connected, but Walmart shows the signed-out page.")
                print("Sign in in the open browser window (keep it OPEN), then re-run --login.")
            else:
                print("Success: connected to your signed-in Walmart session.")
                print("Keep that browser window OPEN, then run the diagnostics or pilot.")
            self.close()
            return
        print("Opening Walmart.com in a dedicated supervised browser profile.")
        ...
        if site.looks_signed_out(page):
            ...
            print("Sign in in the browser, then run:  python walmart_receipts.py --login")
        else:
    ...
    def cmd_discover(self, types: Optional[List[str]] = None, quiet: bool = False) -> dict:
        ...
        for ptype in types:
            ...
            if ptype == IN_STORE and not found_tab:
                ...
                continue
            n_new = 0
            log.info("%s: url=%s", ptype, page.url)
            raw_cards = site.iter_all_cards(page, ptype, delay_ms=int(
                self.config["delay_min_seconds"] * 1000))
            log.info("%s: %d cards collected across all pages", ptype, len(raw_cards))
            for card in raw_cards:
                purchase = site.card_to_purchase(card, ptype)
                if not purchase:
                    log.info("%s: unparseable card id=%r text=%r",
                             ptype, getattr(card, "order_id", ""), (card.text or "")[:120])
                    continue
                key = purchase.key
        ...
        if not quiet:
            ...
            if dates:
                ...
                print("  (If Walmart limits history depth, older purchases simply were")
                print("   not shown by Walmart and are NOT included.)")
        return counts
    ...
    def _save_receipt(self, page, purchase: Purchase) -> bool:
        """Save the order document as a verified PDF, dialog-free.

        Walmart embeds the document in the live details page with print-only
        CSS: in-store trips show a register-style receipt, online orders show
        an invoice. CDP printToPDF (print media) reproduces it cleanly and
        hides the site chrome. We never click 'View receipt details' / 'Print
    ...
    def cmd_run(self, types: List[str], mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            print("This will download your FULL available Walmart purchase history")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Local supervised Walmart receipt downloader")
    modes = [
        ("login", "open browser for manual Walmart sign-in"),
